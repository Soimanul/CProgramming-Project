...
typedef struct {
    ...
    int client_count;
    char creator_username[50];  // Add this line to store the creator's username
} ChatRoom;
...
void broadcast_message(const char *message, int exclude_sock);  // Broadcast message to clients in the same room (excluding a specific socket)
...
void create_chat_room(const char *room_name, Client *creator, char *users);  // Create a new chat room
void room_message(const char *room_name, const char *message, const char *sender);  // Send a message to all clients in a room
